...
class Performance_Measure__Session(Type_Safe):
    ...
    def assert_time(self, *expected_time: int):                                              # Assert that the final score matches the expected normalized time"""
        if in_github_action():
            ...
            new_expected_time   = first_expected_time *3
            assert first_expected_time <=  self.result.final_score <= first_expected_time, f"Performance changed for {self.result.name}: expected {first_expected_time} < {self.result.final_score:,d}ns, expected {first_expected_time}"
        else:
