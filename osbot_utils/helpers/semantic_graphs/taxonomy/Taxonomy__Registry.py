...
class Taxonomy__Registry(Type_Safe):                                                  # Registry for taxonomy definitions
    taxonomies_by_ref : Dict__Taxonomies__By_Ref                                      # Lookup by reference name
    taxonomies_by_id  : Dict__Taxonomies__By_Id                                       # Lookup by instance ID
    ...
    def create_with__random_id(self                                ,
                               ...
                               categories    : Dict__Categories__By_Ref = None) -> Schema__Taxonomy:
        ...
        taxonomy    = Schema__Taxonomy(taxonomy_id    = taxonomy_id                 ,
                                       ...
                                       categories     = categories    or Dict__Categories__By_Ref())
    ...
    def register(self, taxonomy: Schema__Taxonomy) -> Schema__Taxonomy:               # Register taxonomy in both indexes
        self.taxonomies_by_ref[taxonomy.taxonomy_ref] = taxonomy
        if taxonomy.taxonomy_id:                                                      # Only index if ID is set
            self.taxonomies_by_id[taxonomy.taxonomy_id] = taxonomy
        return taxonomy

    @type_safe
    def get_by_ref(self, taxonomy_ref: Taxonomy_Ref) -> Schema__Taxonomy:             # Lookup by reference name
        return self.taxonomies_by_ref.get(taxonomy_ref)

    @type_safe
    def get_by_id(self, taxonomy_id: Taxonomy_Id) -> Schema__Taxonomy:                # Lookup by instance ID
