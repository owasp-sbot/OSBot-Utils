...
## Part 5: Schema Updates (Pure Data)
...
## Part 7: External Seed Cache (Optional)
...
### 7.3 Using External Cache with Registry
...
## Part 8: Collection Subclass Updates
...
### 8.3 New Collections for ID Lookups
...
### 9.1 New Files in `osbot_utils/type_safe/primitives/`
...
### 9.2 Renamed Files in `osbot_utils/helpers/semantic_graphs/`
...
- [ ] Update `Schema__Ontology` — add sidecar, update refs, NO complex __init__
...
### 11.1 Creating Ontology with Random ID
...
### 11.2 Creating Ontology with Deterministic ID (Inline Source)

```python
registry = Ontology__Registry()

uri = Safe_Str__Id__Seed('http://osbot.dev/ontologies/call_flow/1.0')
...
# ontology.ontology_id_source.seed = 'http://osbot.dev/ontologies/call_flow/1.0'
```

### 11.3 Creating Ontology with Explicit ID (External Source)
...
### 11.5 Loading from JSON (Pure Deserialization)

```python
# JSON contains all necessary data
json_data = {
    "ontology_id": "a3f2b8c1",
    "ontology_id_source": {
        "source_type": "deterministic",
        "seed": "http://osbot.dev/ontologies/call_flow/1.0"
    },
...
### 11.6 Node with Deterministic ID from Qualified Name
...
def test__roundtrip(self):
    ...
    original = Schema__Ontology(
        ontology_id        = Ontology_Id(Obj_Id.from_seed('test')),
        ontology_id_source = Ontology_Id__Source(
            source_type = Enum__Id__Source_Type.DETERMINISTIC,
            seed        = Safe_Str__Id__Seed('test'),
        ),
