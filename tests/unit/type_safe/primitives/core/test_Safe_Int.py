...
class test_Safe_Int(TestCase):
    ...
    def test_none_handling(self):
        ...
        assert token_limit == DEFAULT__VALUE_UINT__LLM__MAX_TOKENS      # DEFAULT__VALUE_UINT__LLM__MAX_TOKENS is used as default
        assert token_limit == 5000                                      # in this case 5000
    ...
    def test_sub_handling(self):
        ...
        with pytest.raises(ValueError, match="Safe_UInt__LLM__Max_Tokens must be >= 1, got 0"):
            result = token_limit - DEFAULT__VALUE_UINT__LLM__MAX_TOKENS

        # Augmented assignment ALSO maintains type and validates!
        with pytest.raises(ValueError, match="Safe_UInt__LLM__Max_Tokens must be >= 1, got 0"):
            token_limit -= DEFAULT__VALUE_UINT__LLM__MAX_TOKENS  # ✓ We CAN catch this!
